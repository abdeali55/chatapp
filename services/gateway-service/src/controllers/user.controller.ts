...
import { getAuthenticateUser } from "@/utils/auth";
import { createUserSchema, SearchUserQuery, searchUserQuerySchema, userIdParamsSchema } from "@/validation/user.schema";
...
export const searchUsers: AsyncHandler = async (req, res, next) => {
  try {
    const user = getAuthenticateUser(req);
    const parsedQuery: SearchUserQuery = searchUserQuerySchema.parse(req.query);
