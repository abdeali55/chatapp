import { AUTH_EVENT_EXCHANGE, AUTH_USER_REGISTERED_ROUTING_KEY, type AuthUserRegisteredPayload } from "@chatapp/common";
...
export const publishUserRegistered = (payload: AuthUserRegisteredPayload ) => {
    ...
    if (!published) {
        ...
        logger.info({event}, 'User registered event published');
    }
};
...
export const closePublisher = async () => {
    try {
        ...
        logger.error({err: error}, 'Failed to close RabbitMQ publisher');
    }
};
